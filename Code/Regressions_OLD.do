...
local regression_condition_t "sex!=. & age1!=. & mar_cohab!=. & durats1!=. & fpt_job1!=. & fpt_job2!=. & temporary1!=. & temporary2!=. & public1!=. & public2!=. & selfe1!=. & selfe2!=. & edulevel!=. & seek_method!=. & uresmc1!=. & f_v_retire2!=.  & angSep_CASCOT!=. & industry!=. & jobMover==1" 
local controls_t = "Aggre_Ur_pct Devia_Ur_pct age1 age1_sq mar_cohab n_child durats1 sex fpt_job2 temporary2 i.edulevel i.seek_method i.uresmc1 i.f_v_retire2 lookfor1"
/*
...
capture drop angSep_CASCOT_stdz_t
...
capture drop modOfMod_CASCOT_stdz_t
...
*Probit
capture gen careerChange_angSep = 0
replace careerChange_angSep =1 if angSep_CASCOT >0 & empmon2>0 & empmon2<=3 


capture gen careerChange_angSep_AGG = 0
replace careerChange_angSep_AGG =1 if angSep_CASCOT_AGG >0


gen SOC_1digit_1 = "."
replace SOC_1digit_1 = substr(string(soc10m1),1,1) if date <yq(2020,1) & date>yq(2010,4)
replace SOC_1digit_1 = substr(string(soc2km1),1,1) if date <yq(2011,1) & date>yq(2001,2)

gen SOC_1digit_2 = "."
replace SOC_1digit_2 = substr(string(soc10m2),1,1) if date <yq(2020,1) & date>yq(2010,4)
replace SOC_1digit_2 = substr(string(soc2km2),1,1) if date <yq(2011,1) & date>yq(2001,2)


gen careerChange_1digit = .
replace careerChange_1digit = 0 if empmon2>0 & empmon2<=3 
replace careerChange_1digit = 1 if SOC_1digit_1!=SOC_1digit_2 & SOC_1digit_1!="." &SOC_1digit_2!="." & empmon2>0 & empmon2<=3 

*/
/*
gen careerChange_angSep = .
replace careerChange_angSep = 0 if  empmon2>0 & empmon2<=3
replace careerChange_angSep =1 if angSep_CASCOT >0 & empmon2>0 & empmon2<=3
*/
*xi: probit careerChange_1digit `controls_t' if `regression_condition_t' [pw=lgwt], vce(robust)
*eststo angSep_probit_CASCOT_t

local regression_condition_dh "angSep_CASCOT!=. & Aggre_Ur_pct!=. & sex!=. & age1!=. & mar_cohab!=. & durats1!=. & fpt_job1!=. & temporary1!=. & public1!=. & selfe1!=. & edulevel!=. & uresmc1!=. & quarter!=. & f_v_retire2!=. & industry!=.  & Devia_Ur_pct!=. & n_child!=. & lookfor1!=."
...
xi: probit careerChange_angSep `controls_dh2' if `regression_condition_dh' [pw=lgwt], vce(robust)
eststo angSep_probit_CASCOT_t

/*

* Tobit on aggregated one-digit
local regression_condition_AGG "sex!=. & age1!=. & mar_cohab!=. & durats1!=. & fpt_job1!=. & fpt_job2!=. & temporary1!=. & temporary2!=. & public1!=. & public2!=. & selfe1!=. & selfe2!=. & edulevel!=. & seek_method!=. & uresmc1!=. & f_v_retire2!=.  & angSep_CASCOT_AGG!=. & industry!=. & jobMover==1" 
...
*xi: probit careerChange_angSep `controls_t' if `regression_condition_t' [pw=lgwt], vce(robust)
*eststo angSep_probit_CASCOT_t


xi: probit careerChange_angSep_AGG `controls_t' if `regression_condition_t' [pw=lgwt], vce(robust)
eststo angSep_probit_CASCOT_t
/*		
		* APE (F(z)) - fraction of obs above the limit
		scalar APE = e(N_unc)/(e(N_lc)+e(N_unc))
...
***************************************** proper modOfMod Tobit *****************************************
	...
	***************************************** aggregated angSep Tobit *****************************************		
	...
	eststo angSep_tobit_CASCOT_AGG4
